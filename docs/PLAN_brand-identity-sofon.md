...
4. Проверить факты по каждому продукту с пользователем перед финализацией (парсер не
   ...
   > **Обновление 2026-07-22 (тактика двухфазного наполнения, зафиксировано пользователем):**
   > сначала собираем ВСЕ страницы карты сайта (§8 sitemap v2), заполняя их существующим
   > русским текстом со старого сайта (`docs/legacy-site/pages/*.html`) как есть — без
   > рерайта и без перевода — просто чтобы вся структура сайта стала осязаемой и рабочей.
   > Полный рерайт UA (основной тон бренда) + перевод EN — отдельный, более поздний проход,
   > когда все страницы уже существуют в собранном виде. До этого прохода сайт временно
   > остаётся RU-only по факту, хотя итоговая цель (UA основной + EN) не меняется.
...
**Открытые вопросы трека «сайт» — РЕШЕНЫ пользователем 2026-07-22:**
- ~~Формы (заявка/контакты)~~ — на этом этапе делаем нерабочий макет формы (вёрстка полей,
  без обработчика/сервиса); функционал (mailto/Formspree/Getform/backend) подключаем позже,
  отдельным проходом.
- ~~Домен~~ — остаётся `sofon.com.ua`. По готовности нового сайта старый сносится, новый
  заливается на то же место — не параллельный домен, а замена.
- ~~Информационная архитектура (§8, Фаза 5 v2)~~ — считаем рабочей версией, дальше пока не
  меняем (несмотря на более ранние слова пользователя о «структуру чуть поменяем» — на
  2026-07-22 решено этот вопрос не трогать, работать по v2 как есть).
