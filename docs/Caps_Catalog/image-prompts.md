...
⚠ **Поділ на класи — чернетка, а не структура** (рішення власника 07-08):

> «Поки наперед не діли на класи, ти їх не розумієш до кінця. Поки йдемо по всіх
> доступних типах пробки, напрацьовуємо текст».

Вісім класів нижче придумав я, і придумав **до того**, як власник пройшов типи
по одному. Уже на першому проході виявилось, що я неправильно зібрав кронен із
кільцем, а рядок про демпфірування приписав не тій кришці. Тому класи лишаються
**робочою розкладкою для картинок** — по одній на групу схожих виробів, — але
структурою каталогу вони не є, і текст під них не підганяємо.

*Порядок роботи: спершу проходимо всі типи по одному й напрацьовуємо текст,
класи складаються потім — із того, що виявилось спільним, а не з того, що я
припустив наперед. Це те саме правило, що вже здобуте на межах сторінок:
**межу проводимо після того, як матеріал зібрано, а не до.***
...
## Розкладка, яка спрацювала ✅

**Результат перевірено на Т-подібних, 07-08.** Але текст промта нижче —
⚠ **моя реконструкція, а не оригінал**: власник сказав, що формулював інакше.
⬛ `[ вставити справжній промт ]`

```
Scatter the items loosely in a pile rather than in a grid. Lay about half of
them on their side so the profile of each item is clearly visible, and leave the
rest standing. Vary the sizes noticeably.
```

*Чому саме так. Перша спроба була рівною сіткою — усе стояло вертикально, і
пробки читались як «круглі кришечки»: видно колір, не видно форми. Друга,
врозсип і половина на боці, показала те, заради чого картинка й потрібна, —
вузьку ніжку й широку головку. **Розкладка важливіша за освітлення:** на
довідковому кадрі має бути видно конструкцію, а не палітру.*

⚠ **Блискітка-артефакт.** У правому нижньому куті вона вилізла двічі поспіль,
попри пряму заборону в промті. На третьому кадрі її не було. Якщо з'явиться —
не переробляти кадр, а зрізати кут.
...
✅ **Клас закритий 07-08.** Кадр згенеровано власником і прийнято: сіре тло,
купа врозсип, половина на боці, профіль читається, артефактів немає. Саме він
став еталоном розкладки для решти класів.

⬛ `[ файл покласти в images/t-cork.jpg і дописати рядок в ATTRIBUTION.md:
модель, дата, «ілюстрація, згенеровано» ]`
